...
# 파일 저장하기 전에 Big Five 특성별 군집 분류 추가
big_five_columns = ['openness', 'agreeableness', 'emotional_stability', 
                    'conscientiousness', 'extraversion']

# 각 사용자별로 가장 높은 성격 특성 찾기
def get_dominant_trait(row):
    trait_values = row[big_five_columns]
    max_trait = trait_values.idxmax()
    return max_trait

# 지배적인 성격 특성 컬럼 추가
final_df_encoded['dominant_trait'] = final_df_encoded[big_five_columns].apply(get_dominant_trait, axis=1)

# 군집별 사용자 수 확인
trait_distribution = final_df_encoded['dominant_trait'].value_counts()
print("\n성격 특성별 사용자 분포:")
print(trait_distribution)

# 각 군집의 평균 특성 값 확인
trait_means = final_df_encoded.groupby('dominant_trait')[big_five_columns].mean()
print("\n군집별 평균 특성 값:")
print(trait_means)
